...
- Agent job queue foundation: lead website research, lead draft generation, and mailbox suggested replies can now be queued as AgentJobs, listed in Automation, and processed through reusable processor services shared with the inline Agent 1/2/3 and mailbox flows. The processor supports signed-in manual runs plus `AGENT_JOB_WORKER_SECRET` cron/worker calls with bounded batch limits.
...
- Full background worker coverage for provider-backed Agent execution. Durable AgentJob records and worker-authenticated processing now exist for lead website research, draft generation, and mailbox reply drafting; remaining work is wiring the external scheduler and adding job types such as partner search and Gmail sync.
...
- Deeper mailbox automation. A dedicated mailbox UI surface and queued reply drafting now exist; remaining gaps are send-as aliases, richer automated classification over synced Gmail messages, and background Gmail sync/send queue handling.
...
1. Wire deployment cron to `/api/agent-jobs/process` with `AGENT_JOB_WORKER_SECRET`, then expand job types to partner search, Gmail sync, and mailbox reply drafting.
