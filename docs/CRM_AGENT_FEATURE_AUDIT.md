...
- Full background worker coverage for provider-backed Agent execution. Durable AgentJob records and worker-authenticated processing now exist for lead website research, draft generation, mailbox reply drafting, Gmail mailbox sync/send, and provider-backed partner sourcing; remaining work is wiring the external scheduler and adding full pipeline orchestration.
