...
- Agent job queue foundation: lead draft generation can now be queued as an AgentJob, listed in Automation, and processed through a reusable processor endpoint that shares the inline Agent 2/3 draft service. The processor supports signed-in manual runs plus `AGENT_JOB_WORKER_SECRET` cron/worker calls with bounded batch limits.
...
- Full background worker coverage for provider-backed Agent execution. Durable AgentJob records and worker-authenticated processing now exist for lead draft generation; remaining work is wiring the external scheduler and adding job types such as website research, partner search, Gmail sync, and mailbox reply drafting.
...
1. Wire deployment cron to `/api/agent-jobs/process` with `AGENT_JOB_WORKER_SECRET`, then expand job types to website research, partner search, Gmail sync, and mailbox reply drafting.
