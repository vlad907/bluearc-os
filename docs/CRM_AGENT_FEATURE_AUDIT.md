...
- Agent job queue foundation: lead website research, lead draft generation, combined lead research-to-draft orchestration, mailbox suggested replies, Gmail mailbox sync/send, and partner sourcing can now be queued as AgentJobs, listed in Automation, and processed through reusable processor services shared with the inline Agent 1/2/3, mailbox, Gmail, and partner-search flows. The processor supports signed-in manual runs plus `AGENT_JOB_WORKER_SECRET` cron/worker calls with bounded batch limits.
...
- Full background worker coverage for provider-backed Agent execution. Durable AgentJob records and worker-authenticated processing now exist for lead website research, draft generation, a combined lead research-to-draft pipeline, mailbox reply drafting, Gmail mailbox sync/send, and provider-backed partner sourcing; remaining work is wiring the external scheduler and expanding pipeline orchestration across imported leads and mailbox reply chains.
...
- Deeper mailbox automation. A dedicated mailbox UI surface, queued Gmail sync/send, and queued reply drafting now exist; remaining gaps are send-as aliases, richer automated classification over synced Gmail messages, and full reply pipeline orchestration.
...
- Background pipeline worker for imported → research → draft → verify → draft/send progression. The first combined lead research-to-draft job exists; remaining work is automatic trigger rules across imported leads and mailbox send chains.
...
1. Wire deployment cron to `/api/agent-jobs/process` with `AGENT_JOB_WORKER_SECRET`, then expand from discrete jobs into full pipeline orchestration.
